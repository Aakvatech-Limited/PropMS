...
def get_data(filters):
	...
	for invoice in sales_invoices:
		...
		if invoice.total == invoice.foreign_total:
			invoice.exchange_rate = get_exchange_rate("USD", default_currency, invoice.posting_date)
...
def get_columns(filters):
	...
	columns = [
		...
		{
			"label": f"Advance Before {_foreign_currency}",
		...
		{
			"label": f"Item Total {_foreign_currency}",
	...
	for month in months_list:
		columns.append(
			{
				"label": f"{month} {currency}",
				"fieldname": f"{month.lower()} {currency}",
				"fieldtype": "Float",
				"width": 100,
			}
		)
		if filters.get("foreign_currency") and filters.get("foreign_currency") != currency:
			columns.append(
				{
					"label": "{} {}".format(month, filters.get("foreign_currency")),
					"fieldname": "{} {}".format(month.lower(), filters.get("foreign_currency")),
					"fieldtype": "Float",
					"width": 100,
				}
			)
	...
	columns.append(
		{
			"label": f"Advance After {_foreign_currency}",
...
def get_months(from_date, to_date):
	...
	months_obj = OrderedDict(
		((start + timedelta(_)).strftime(r"%b-%y"), None) for _ in range((end - start).days)
	)
	for key, _value in months_obj.items():
...
def calculate_monthly_ammount(
	...
	for month in get_months(filters["from_date"], filters["to_date"]):
		months_report_list.append(f"{month.lower()} {default_currency}")
		if filters.get("foreign_currency") and filters.get("foreign_currency") != default_currency:
			months_report_list.append("{} {}".format(month.lower(), filters.get("foreign_currency")))
		...
		while date <= end_date:
			...
			if start_month == end_month:
				...
				field_list.append(
					{
						"days_diff": days_diff,
						"month_filed": f"{month_filed} {default_currency}",
						"month_len": month_len,
						"foreign": False,
					}
				)
				if foreign_currency and foreign_currency != default_currency:
					field_list_foreign.append(
						{
							"days_diff": days_diff,
							"month_filed": f"{month_filed} {foreign_currency}",
							"month_len": month_len,
							"foreign": True,
						}
					)
		...
		for i in field_list_foreign:
			if n == 1 and i["days_diff"] < 30:
				monthly_ammount_obj[i["month_filed"]] = flt(
					i["days_diff"] * ((foreign_ammount - sub_ammount_foreign) / first_last),
					float_precision,
				)
