...
frappe.ui.form.on('Meter Reading Detail', {
	...
	if(doc.property && cur_frm.doc.meter_type){
		frappe.call({
			...
			args:{'property_id':doc.property,'meter_type':cur_frm.doc.meter_type},
			callback:function(r){
				if(r.message){
					...
					frappe.model.set_value(cdt,cdn,"previous_meter_reading","")
					frappe.throw("Meter Does Not Exist For This Type")
				}
			}
		})
	...
	var doc = locals[cdt][cdn]
	console.log(doc.meter_number)
	console.log(doc.property)
	console.log(cur_frm.doc.meter_type)
	if(doc.meter_number && doc.property && cur_frm.doc.meter_type){
		frappe.call({
			method:"propms.auto_custom.get_previous_meter_reading",
			args:{'meter_number':doc.meter_number,'property_id':doc.property,'meter_type':cur_frm.doc.meter_type},
			callback:function(r){
				if(r.message){
	...
	if(doc.previous_meter_reading && doc.current_meter_reading){
