		...
		for item_child in self.items:
			if not item.name == item_child.name:
				if item.item_code == item_child.item_code:
					frappe.throw(
						_("Duplicate Item Exists - {0}. Duplications are not allowed.").format(item.item_code)
					)
...
def statusChangeBeforeLeaseExpire():
	try:
		# ---------------------------------------------
		# Mark properties as "Off Lease in 3 Months"
	...
	try:
		...
		for prop in properties:
			...
			active_leases = (
				...
				.where((Lease.end_date >= frappe.utils.now()) | (Lease.end_date.isnull()))
			).run()
...
def update_property_status():
	try:
		...
		for row in active_lease_properties:
			frappe.db.set_value("Property", row.name, "status", "On Lease")
	...
	while start_date <= end_date:
		...
		if period_end_date <= end_date:
			...
			no_days_in_month = float(
				calendar.monthrange(getdate(start_date).year, getdate(start_date).month)[1]
			)
